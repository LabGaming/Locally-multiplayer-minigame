...
public class PlayerController2D : MonoBehaviour
{
	...
	private PlayerActions myPlayerActions;

    // Use this for initialization
    void Start()
    {
		myPlayerActions = transform.GetComponent<PlayerActions> ();
    ...
    void Update()
    {
		...
		if (Input.GetButtonDown(myPlayerActions.inputConfig.JUMP))
        {
            _motor.Jump();
            _motor.DisableRestrictedArea();
        }

		_motor.jumpingHeld = Input.GetButton(myPlayerActions.inputConfig.JUMP);

        // XY freedom movement
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            //_motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL);
			_motor.normalizedXMovement = Input.GetAxis(myPlayerActions.inputConfig.HORIZONTAL);
			_motor.normalizedYMovement = Input.GetAxis(myPlayerActions.inputConfig.VERTICAL);

            return; // do nothing more
        }

        // X axis movement
		if (Mathf.Abs(Input.GetAxis(myPlayerActions.inputConfig.HORIZONTAL)) > PC2D.Globals.INPUT_THRESHOLD)
        {
			_motor.normalizedXMovement = Input.GetAxis(myPlayerActions.inputConfig.HORIZONTAL);
        }
        else
		...
		if (Input.GetAxis(myPlayerActions.inputConfig.VERTICAL) != 0)
        {
			bool up_pressed = Input.GetAxis(myPlayerActions.inputConfig.VERTICAL) > 0;
		...
		else if (Input.GetAxis(myPlayerActions.inputConfig.VERTICAL) < -PC2D.Globals.FAST_FALL_THRESHOLD)
        {
            _motor.fallFast = false;
        }

		if (Input.GetButtonDown(myPlayerActions.inputConfig.DASH))
        {
            _motor.Dash();
        }
    }
